...
public class BetweenCliOptionsParser {
    ...
    public static final Option START_TIME = Option
            ...
            .numberOfArgs(1)
            .argName("start-time")
            .desc("a start-time must enter")
            .build();

    public static final Option END_TIME = Option
            .builder("etime")
            .required(true)
            .longOpt("end-time")
            .numberOfArgs(1)
            .argName("end-time")
            .desc("a end-time must enter")
            .build();
    ...
    public static CliOptions parseEmbeddedModeClient(String[] args) {
        try {
            ...
            return new CliOptions(
                    line.hasOption(BetweenCliOptionsParser.OPTION_HELP.getOpt()),
                    checkBootstrapServer(line),
                    checkGroupId(line),
                    checkTopicName(line),
                    checkStartTime(line),
                    checkEndTime(line),
                    line.getOptionValue(BetweenCliOptionsParser.KEY_DESERIALIZE.getOpt(), "org.apache.kafka.common.serialization.StringDeserializer"),
                    line.getOptionValue(BetweenCliOptionsParser.VALUE_DESERIALIZE.getOpt(), "org.apache.kafka.common.serialization.StringDeserializer"));
        }
...
//            printHelpClient();
    ...
    public static String checkEndTime(CommandLine line){
        final String startTime = line.getOptionValue(BetweenCliOptionsParser.END_TIME.getOpt());
    ...
    public static String checkStartTime(CommandLine line){
        final String startTime = line.getOptionValue(BetweenCliOptionsParser.START_TIME.getOpt());
        if(null == startTime|| startTime.isEmpty()){
            throw new LoadKafkaOffsetException("Kafka Consumer' Start Time Must Enter");
        }
        return startTime;
    ...
    public static String checkTopicName(CommandLine line){
        final String topic = line.getOptionValue(BetweenCliOptionsParser.TOPIC_ANME.getOpt());
    ...
    public static String checkGroupId(CommandLine line){
        final String groupId = line.getOptionValue(BetweenCliOptionsParser.GROUP_ID.getOpt());
    ...
    public static String checkBootstrapServer(CommandLine line){
        final String bootstrapServer = line.getOptionValue(BetweenCliOptionsParser.BOOTSTARP_SERVER.getOpt());
